...
const LabDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // Fetch bookings
        const bookingsResponse = await fetch(`${API_BASE_URL}/bookings/lab/${user.id}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const bookingsData = await bookingsResponse.json();

        // Fetch lab details including time slots
        const labResponse = await fetch(`${API_BASE_URL}/labs/${user.id}/tests`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const handleSaveTimeSlots = async () => {
    ...
    try {
      const token = getToken();
      const response = await fetch(`${API_BASE_URL}/labs/${user.id}/time-slots`, {
        method: 'PUT',
        headers: {
