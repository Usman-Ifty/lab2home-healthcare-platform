...
const PhlebotomistMessages = () => {
  ...
  const [chatType, setChatType] = useState<'patient' | 'lab'>('patient');
  ...
  const [stats, setStats] = useState({ todayVisits: 0, completedToday: 0, pendingToday: 0 });
  ...
  useEffect(() => {
    if (token && user) {
      ...
      // Fetch Bookings to find available patients, labs and stats
      phlebotomistService.getAssignedBookings().then((data) => {
        if (data.success) {
          ...
          setStats({
            todayVisits: todayBookings.length,
            completedToday: todayBookings.filter((b: any) => b.status === 'completed').length,
            pendingToday: todayBookings.filter((b: any) => ['pending', 'confirmed', 'in-progress'].includes(b.status)).length,
          });
        }
      });
    }
  ...
  const handleChatSelect = async (contactId: string) => {
    ...
    const existingConv = conversations.find(c =>
      chatType === 'patient' ? c.patient?._id === contactId : c.lab?._id === contactId
    );
  ...
  // Switch chat type
  const handleChatTypeChange = (type: 'patient' | 'lab') => {
    setChatType(type);
  ...
  const currentContacts = chatType === 'patient' ? availablePatients : availableLabs;
      ...
      <div className="flex flex-col gap-6 h-[calc(100vh-80px)]">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Coordinate with patients and labs for home visits and sample collections.
          </p>
        </div>
          ...
          <Card className="lg:col-span-2 p-4 rounded-xl shadow-md bg-card flex flex-col h-full overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold flex items-center gap-2">
                ...
                {selectedChatId ? `Chat with ${chatType === 'patient' ? selectedConversation?.patient?.fullName : selectedConversation?.lab?.labName}` : `Select ${chatType === 'patient' ? 'Patient' : 'Lab'} to Chat`}
              </h2>
              ...
              >
                <option value="" disabled>Select {chatType === 'patient' ? 'Patient' : 'Lab'}</option>
                {currentContacts.map((contact: any) => (
                  <option key={contact._id} value={contact._id}>
                    {chatType === 'patient' ? contact.fullName : contact.labName}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div ref={chatRef} className="flex-1 overflow-y-auto bg-muted/30 border rounded-lg p-4 space-y-3">
              {messages.map((msg, i) => {
                const isMe = msg.sender === 'phlebotomist';
                return (
            ...
            <Card className="p-5 rounded-xl shadow-md border">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-2xl font-bold">{stats.todayVisits} Visits</p>
                  <p className="text-xs text-muted-foreground mt-1">{stats.completedToday} completed · {stats.pendingToday} pending</p>
                </div>
            ...
            <Card className="p-5 rounded-xl shadow-md border">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-2xl font-bold">{stats.pendingToday > 0 ? 'Scheduled' : 'None'}</p>
                  <p className="text-xs text-muted-foreground mt-1">Confirm via chat if needed</p>
                </div>
